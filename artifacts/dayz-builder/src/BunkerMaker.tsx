...
function FloorPlanSVG({ layout, showLevel, debugMode }: { layout: BunkerLayout; showLevel: number; debugMode?: boolean }) {
  ...
  for (const o of levelObjs) {
    const [hw, hd] = SECTION_HALF[o.section] ?? [5, 5];
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" height="100%"
      ...
      {levelObjs.map((obj, i) => {
        ...
        const color = overlapping.has(i) ? '#e74c3c' : (SECTION_COLORS[obj.section] ?? '#5a4820');
        const [hw, hd] = SECTION_HALF[obj.section] ?? [5, 5];
        const pw = hw * scale * 2;
        const ph = hd * scale * 2;
    ...
    <div className="flex flex-col h-full bg-[#080f09] text-[#b8d4b8] font-mono overflow-hidden">
      ...
      <div className="flex items-center gap-2 px-3 py-2 border-b border-[#1a2e1a] bg-[#0a1209] shrink-0">
        ...
        <div className="ml-auto flex items-center gap-1.5">
          <span className="text-[#5a8a5a] text-[9px]">{layout.stats.totalObjects} objects</span>
            ...
            <div className="flex flex-col gap-1">
              {(Object.keys(SIZES) as BunkerSize[]).map(s => (
                <button key={s} onClick={() => setSize(s)}
                  className={`text-left px-2 py-1.5 text-[10px] rounded-sm border transition-all ${size === s ? 'bg-[#27ae60]/20 border-[#27ae60] text-[#27ae60]' : 'border-[#1a2e1a] text-[#b09a6a] hover:border-[#5a4820]'}`}>
                  {SIZES[s].label}
