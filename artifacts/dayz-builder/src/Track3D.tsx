...
export function buildFloorTiles(waypoints: Pt[], trackWidth: number): TileXfm[] {
  ...
  for (let i = 0; i < n; i++) {
    ...
    // DayZ is clockwise, Three.js is counter-clockwise, so we must invert yaw.
...
export function buildBarriers(waypoints: Pt[], trackWidth: number, barrierLen: number): TileXfm[] {
  ...
  for (let i = 0; i < n; i++) {
    ...
    // Barrier long-axis (Z=6m) must align ALONG the track — same as floor tiles
    const yr = -yaw * Math.PI / 180;

    // Match the improved chaining logic from RaceTrackMaker.tsx
    const OVERLAP = 0.45; 
    const effectiveLen = barrierLen - OVERLAP;
    const nb = Math.max(1, Math.round(len / effectiveLen));
    const actualStep = len / nb;
...
function Barriers({ xfms }: { xfms: TileXfm[] }) {
  ...
  const mat = <meshStandardMaterial color="#cac3bb" roughness={0.4} metalness={0.6} emissive="#222222" emissiveIntensity={0.1} />;
  ...
  return (
    <>
      ...
      {/* Red safety stripe on top with subtle glow */}
      <instancedMesh ref={stripeRef} key={`bs${xfms.length}`} args={[undefined, undefined, xfms.length]} frustumCulled={false}>
        ...
        <meshStandardMaterial color="#c0392b" roughness={0.3} metalness={0.8} emissive="#ff0000" emissiveIntensity={0.4} />
      </instancedMesh>
    </>
  );
}
...
function Track3DScene({ waypoints, trackWidth, addBarriers, addText, barrierLen, hazardObjects }: Track3DSceneProps) {
  ...
  return (
    <>
      ...
      <directionalLight position={[100, 150, 80]} intensity={2.2} color="#ffffff" castShadow />
      <directionalLight position={[-80, 50, -100]} intensity={1.2} color="#27ae60" />
      <pointLight position={[0, 20, 0]} intensity={1.5} color="#27ae60" distance={200} decay={2} />
