...
export function generateBuildInterior(
  ...
  // ── Interior walls (structure type only) ──────────────────────────────────
  if (interiorType === "structure") {
  ...
  // ── Decorative props ──────────────────────────────────────────────────────
  const propCount = interiorType === "structure" ? 6 + Math.floor(rng() * 7)  // 6–12
  ...
  for (let i = 0; i < propCount; i++) {
    const isWallProp = interiorType === "structure" && rng() > 0.5;
  ...
  // ── Loot items ────────────────────────────────────────────────────────────
  const pool = (LOOT_BY_TYPE as any)[lootTheme] ?? LOOT_BY_TYPE.mixed;
