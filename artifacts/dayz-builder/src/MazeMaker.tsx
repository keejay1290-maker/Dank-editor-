...
function MazePreview({ seed, size, roomSz }: { seed: number; size: number; roomSz: number }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full max-w-[400px] mx-auto" style={{ background: "#080f09" }}>
      <rect x={PAD} y={PAD} width={cellW*cpx} height={cellH*cpx} fill="#0c1510" />
      <rect x={rxPx} y={ryPx} width={rw} height={rh} fill="#2a2000" stroke="#27ae60" strokeWidth={1.5} />
      <text x={rxPx+rw/2} y={ryPx+rh/2+4} textAnchor="middle" fill="#27ae60"
      ...
      <text x={PAD+10} y={PAD-1} fill="#3a6a3a" fontSize="8">Entry</text>
      <rect x={PAD+44} y={PAD-6} width={8} height={6} fill="#e74c3c" rx="1"/>
      <text x={PAD+54} y={PAD-1} fill="#3a6a3a" fontSize="8">Exit</text>
    </svg>
  );
}
...
export default function MazeMaker() {
  ...
  const output = useMemo(() => {
    ...
    const lines = [
      ...
      ``,
      `// ===================================================`,
      `// MAZE ARENA -- Generated by Dank's Dayz Studio`,
      `// ===================================================`,
  ...
  const NumInput = ({ label, value, onChange }: { label: string; value: number; onChange: (v: number) => void }) => (
    <div className="flex-1">
      <div className="text-[#5a8a5a] text-[9px] mb-0.5">{label}</div>
      <input type="number" value={value} onChange={e => onChange(Number(e.target.value))}
        className="w-full bg-[#1a1610] border border-[#1a2e1a] text-[#b8d4b8] text-[11px] px-2 py-1 rounded-sm focus:outline-none focus:border-[#27ae60]" />
    </div>
  );
  ...
  const SidebarContent = () => (
    <div className="p-3 space-y-4">
      ...
      {/* Seed */}
      <div>
        <div className="text-[#5a8a5a] text-[9px] uppercase tracking-wider mb-2">🌀 Maze Seed</div>
        <div className="flex gap-1.5 mb-1.5">
          <input type="number" min={1} max={9999} value={seed}
            onChange={e => setSeed(Math.max(1, Math.min(9999, Number(e.target.value))))}
            className="flex-1 bg-[#1a1610] border border-[#1a2e1a] text-[#b8d4b8] text-[11px] px-2 py-1.5 rounded-sm focus:outline-none focus:border-[#27ae60]" />
          <button onClick={rollSeed}
            className="px-3 py-1.5 bg-[#27ae60] text-[#080f09] text-[10px] font-bold rounded-sm hover:bg-[#e8b82a] transition-colors">
            🎲 ROLL
          </button>
        </div>
      ...
      {/* Layout */}
      <div>
        <div className="text-[#5a8a5a] text-[9px] uppercase tracking-wider mb-2">📐 Layout</div>
        <Slider
          label="Grid Size"
          value={size}
      ...
      {/* Wall settings */}
      <div>
        <div className="text-[#5a8a5a] text-[9px] uppercase tracking-wider mb-2">🧱 Wall Object & Scale</div>
        <select value={wallObj} onChange={e => setWallObj(e.target.value)}
          className="w-full bg-[#1a1610] border border-[#1a2e1a] text-[#b8d4b8] text-[10px] px-2 py-1.5 rounded-sm focus:outline-none focus:border-[#27ae60] mb-2">
          {WALL_OBJECTS.map(w => (
        ...
        {/* Efficiency display */}
        <div className="bg-[#0a1209] border border-[#1a2e1a] rounded-sm p-2 text-[9px]">
          <div className="flex justify-between mb-1">
            <span className="text-[#3a6a3a]">1 object per wall segment</span>
      ...
      {/* Position */}
      <div>
        <div className="text-[#5a8a5a] text-[9px] uppercase tracking-wider mb-1.5">📍 World Position</div>
        <div className="flex gap-1.5 mb-1.5">
      ...
      {/* Format */}
      <div>
        <div className="text-[#5a8a5a] text-[9px] uppercase tracking-wider mb-1.5">📄 Output Format</div>
        <div className="flex gap-1">
          {(["initc","json"] as const).map(f => (
            <button key={f} onClick={() => setFormat(f)}
              className={`flex-1 py-1.5 text-[10px] font-bold rounded-sm border transition-colors ${format === f ? "bg-[#27ae60] text-[#080f09] border-[#27ae60]" : "border-[#1a2e1a] text-[#3a6a3a] hover:border-[#27ae60] hover:text-[#27ae60]"}`}>
              {f === "initc" ? "init.c" : "JSON"}
            </button>
          ))}
        </div>
      ...
      <div className="border border-[#1a2e1a] rounded-sm p-2 bg-[#0a1209]">
        <div className="text-[#5a8a5a] text-[9px] uppercase tracking-wider mb-1.5">📊 Summary</div>
        <div className="flex flex-wrap gap-1.5">
          <span className="bg-[#1a1610] border border-[#1a2e1a] text-[#27ae60] text-[9px] px-2 py-0.5 rounded-sm font-bold">{spawns.length} objects</span>
          <span className="bg-[#1a1610] border border-[#1a2e1a] text-[#5a8a5a] text-[9px] px-2 py-0.5 rounded-sm">{size}×{size} grid</span>
          <span className="bg-[#1a1610] border border-[#1a2e1a] text-[#5a8a5a] text-[9px] px-2 py-0.5 rounded-sm">~{footprint}×{footprint}m</span>
          <span className="bg-[#1a1610] border border-[#1a2e1a] text-[#5a8a5a] text-[9px] px-2 py-0.5 rounded-sm">{cellSz}m corridor</span>
          <span className="bg-[#1a1610] border border-[#1a2e1a] text-[#5a8a5a] text-[9px] px-2 py-0.5 rounded-sm">Scale {wallScale}×</span>
        </div>
  ...
  const CodePanel = () => (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2 px-3 py-2 border-b border-[#1a2e1a] shrink-0 flex-wrap">
        {(["initc","json"] as const).map(f => (
          <button key={f} onClick={() => setFormat(f)}
            className={`px-3 py-1 text-[10px] font-bold rounded-sm transition-colors ${format === f ? "bg-[#27ae60] text-[#080f09]" : "text-[#3a6a3a] border border-[#1a2e1a] hover:border-[#27ae60]"}`}>
            {f === "initc" ? "init.c" : "JSON"}
          </button>
        ))}
        ...
        <div className="ml-auto flex gap-1.5">
          <button onClick={copyOutput}
            className="px-3 py-1 bg-[#1a1610] border border-[#1a2e1a] text-[#b09a6a] text-[10px] rounded-sm hover:border-[#27ae60] hover:text-[#27ae60] transition-colors">
            Copy
          </button>
          <button onClick={downloadOutput}
            className="px-3 py-1 bg-[#27ae60] text-[#080f09] text-[10px] font-bold rounded-sm hover:bg-[#e8b82a] transition-colors">
            Download
          </button>
        </div>
      </div>
    ...
    <div className="flex flex-1 overflow-hidden bg-[#080f09] relative">
      ...
      <div className="md:hidden absolute top-0 left-0 right-0 flex border-b border-[#1a2e1a] bg-[#0a1209] z-10">
        {(["options","map","code"] as const).map(t => (
          <button key={t} onClick={() => setMobileTab(t)}
            className={`flex-1 py-2 text-[10px] font-bold uppercase tracking-wider transition-colors ${mobileTab === t ? "text-[#27ae60] border-b-2 border-[#27ae60]" : "text-[#3a6a3a]"}`}>
      ...
      <div className={`${mobileTab !== "options" ? "hidden md:flex" : "flex"} w-full md:w-72 shrink-0 bg-[#0a1209] border-r border-[#1a2e1a] overflow-y-auto flex-col md:mt-0 mt-9`}>
        <div className="px-3 py-3 border-b border-[#1a2e1a] shrink-0">
          ...
          <div className="text-[#27ae60] font-bold text-[13px]">🌀 MAZE BUILDER</div>
          <div className="text-[#3a6a3a] text-[9px] mt-0.5">1 scaled wall per segment · max {MAX_OBJ} objects</div>
        </div>
        <SidebarContent />
      </div>

      {/* 3D Map preview */}
      <div className={`${mobileTab !== "map" ? "hidden md:flex" : "flex"} flex-col flex-1 overflow-hidden border-r border-[#1a2e1a] md:mt-0 mt-9`}>
        <div className="px-3 py-2 border-b border-[#1a2e1a] shrink-0 flex items-center gap-2">
          <span className="text-[#5a8a5a] text-[10px] uppercase tracking-wider">3D Preview</span>
          <span className="text-[#4a3a1a] text-[9px]">— {size}×{size} · Seed #{seed} · {spawns.length} walls</span>
          <button onClick={rollSeed}
            className="ml-auto px-2 py-0.5 bg-[#1a1610] border border-[#1a2e1a] text-[#27ae60] text-[9px] rounded-sm hover:border-[#27ae60] transition-colors">
            New Maze
          </button>
        </div>
        ...
        <div className="shrink-0 px-3 py-1.5 border-t border-[#1a2e1a] bg-[#0a1209] flex flex-wrap gap-1.5">
          <span className="bg-[#1a1610] border border-[#1a2e1a] text-[#27ae60] text-[9px] px-2 py-0.5 rounded-sm font-bold">{spawns.length} objects</span>
          <span className="bg-[#1a1610] border border-[#1a2e1a] text-[#5a8a5a] text-[9px] px-2 py-0.5 rounded-sm">{cellSz}m corridors</span>
          <span className="bg-[#1a1610] border border-[#1a2e1a] text-[#5a8a5a] text-[9px] px-2 py-0.5 rounded-sm">~{footprint}×{footprint}m</span>
          <span className="bg-[#1a1610] border border-[#1a2e1a] text-[#27ae60] text-[9px] px-2 py-0.5 rounded-sm">YPR shown in 3D</span>
        </div>
      </div>
