...
export default function DankCoordsTool() {
  ...
  const processBatch = () => {
    ...
    const xmlRegex = /<pos\s+x="([^"]+)"\s+y="([^"]+)"\s+z="([^"]+)"\s*\/>/gi;
    const rpyRegex = /<rpy\s+a="([^"]+)"\s+p="([^"]+)"\s+r="([^"]+)"\s*\/>/gi;
    let match;
    let posIdx = 0;
    while ((match = xmlRegex.exec(batchInput)) !== null) {
      const xVal = parseFloat(match[1]);
      const yVal = parseFloat(match[2]);
      const zVal = parseFloat(match[3]);
      let angle = 0;
      const rMatch = rpyRegex.exec(batchInput);
      if (rMatch) angle = parseFloat(rMatch[1]);
      newMarkers.push({ id: uid(), name: `Imported_${++posIdx}`, x: xVal, y: yVal, z: zVal, a: angle });
    }
    if (newMarkers.length === 0) {
      batchInput.split('\n').forEach((line, i) => {
        ...
        if (parts.length >= 2) {
          newMarkers.push({
            id: uid(), name: `Marker_${i + 1}`,
            x: parts[0], y: parts.length > 2 ? parts[1] : 0,
            z: parts.length > 2 ? parts[2] : parts[1] || 0,
    ...
    <div className="flex flex-col h-screen bg-[#080f09] text-[#b8d4b8] font-mono overflow-hidden">
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        ...
        <div className="w-64 shrink-0 border-r border-[#1a2e1a] bg-[#0a1209] p-3 overflow-y-auto flex flex-col gap-3">
          ...
          {/* BATCH IMPORT */}
          <div className="border border-[#1abc9c]/30 rounded overflow-hidden">
            <button onClick={() => setShowBatch(b => !b)} className={`w-full flex items-center justify-between px-3 py-1.5 transition-all text-[10px] font-black ${showBatch ? "bg-[#1abc9c] text-[#080f09]" : "bg-[#0c1510] text-[#1abc9c] hover:bg-[#0e2010]"}`}>
              <span>🚀 BATCH XML/CSV IMPORT</span><span>{showBatch ? "▲" : "▼"}</span>
            </button>
            {showBatch && (
              <div className="p-3 bg-[#080f09] space-y-2">
                <div className="text-[8px] text-[#3a6a3a] leading-tight">Paste mapgrouppos.xml &lt;pos&gt; tags or raw X,Y,Z lines below.</div>
                <textarea 
                  value={batchInput}
                  onChange={e => setBatchInput(e.target.value)}
                  placeholder="<pos x='123' ... /> or 123 456 789"
                  className="w-full h-32 bg-[#0c1510] border border-[#1a2e1a] text-[#b8d4b8] text-[9px] p-2 font-mono outline-none focus:border-[#1abc9c]"
                />
                <button 
                  onClick={processBatch}
                  className="w-full py-1.5 bg-[#1abc9c] text-[#080f09] text-[10px] font-black rounded hover:bg-white transition-all">
                  PROCESS BATCH
                </button>
              </div>
            )}
